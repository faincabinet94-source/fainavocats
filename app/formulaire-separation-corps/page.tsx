...
function FormContent() {
  ...
  const [iframeUrl, setIframeUrl] = useState("https://www.cognitoforms.com/f/7odepi9SUkCmb7Yrf3m2Cg/13")

  useEffect(() => {
    const entryData = searchParams.get('entry')
    if (entryData) {
      setIframeUrl(`https://www.cognitoforms.com/f/7odepi9SUkCmb7Yrf3m2Cg/13?entry=${encodeURIComponent(entryData)}`)
    }
  }, [searchParams])
  ...
  return (
    <div className="w-full border-t border-slate-100 pt-8">
      <iframe 
        ...
        frameBorder="0"
        scrolling="yes"
        title="Formulaire de séparation de corps"
      ></iframe>
    </div>
  )
}

export default function FormSeparationCorpsPage() {
  return (
